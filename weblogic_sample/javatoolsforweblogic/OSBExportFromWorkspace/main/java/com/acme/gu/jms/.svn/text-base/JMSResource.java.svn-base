...
public class JMSResource {
	...
	public static void testMethod() {
		...
		try {
			...
			env.put(Context.PROVIDER_URL, "t3://iinfblockbcl003.msnet.railb.be:7009");
			...
			for (int i = 0; i < length; i++) {
				...
				for (int k = 0; k < queueCount; k++) {
					...
					JMSSystemResourceMBean jmsSR = null;
					try {
						jmsSR = JMSModuleHelper.findJMSSystemResource(ctx, resourceName);
					}
					catch (Exception e) {
						e.printStackTrace();
					}
					if (jmsSR != null) {
						...
						int qlength = (int) queueBean.length;

						for (int m = 0; m < qlength; m++) {
	...
	public static void main(String[] args) throws Exception {
		...
		String hostname = "iinfblockbcl003.msnet.railb.be";
		String portString = "7009";
